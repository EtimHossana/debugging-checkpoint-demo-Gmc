...
const PilferingPillbug = (props) => {
  ...
  return (
    <>
      ...
      <LikeButton
        //place the spread statement above the the state so it overrides any initial props
        {...props}
        liked={likeStatus}
        likedBy={currentlyLikedBy}
        onLikeChange={handleOnLikeChange}
        //The state of the PilferingPillbug is to being passed to the like button properly
        //because it spreads the initial prop which overrides its state value
        // {...props}
      />
      <BugAttributes
...
//props spreading caused the summary size to change
//to fix this we had to destructure the prop by adding level
//adding spread to htmlprop
function PurchaseSummary({ level, ...htmlprops }) {
  return (
    <>
      <Heading
      ...
      <Text data-test="summary" color="text-weak">
        ...
        {htmlprops.liked ?? "haven't decided if you like or not"}
      </Text>
    </>
  );
}
