...
const LikeButton = ({ liked, onLikeChange }) => {
  ...
  useEffect(() => {
    if (liked === "like" && !hasLikedOnce) {
      setHasLikedOnce(true); // persist one-time mount
    }
  }, [liked, hasLikedOnce]);
  ...
  return (
    <Box direction="row" gap="small">
      ...
      />
      {/* Mount only once, show only if liked */}
      {hasLikedOnce && <AffectionateBadge show={liked === "like"} />}
    </Box>
  );
};
...
function AffectionateBadge({ show }) {
